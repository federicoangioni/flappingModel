...
for i=1:5
    corr7=corrcoef(RD(~isnan(RD(:,i)),i),PD(~isnan(RD(:,i)),i));
    corr8=corrcoef(RD(~isnan(RD(:,i)),i),QD(~isnan(RD(:,i)),i));
    corr9=corrcoef(RD(~isnan(RD(:,i)),i),PD(~isnan(RD(:,i)),i).*U(~isnan(RD(:,i)),i));
    corr10=corrcoef(RD(~isnan(RD(:,i)),i),QD(~isnan(RD(:,i)),i).*V(~isnan(RD(:,i)),i));
    corr11=corrcoef(RD(~isnan(RD(:,i)),i)+Cyaw0*R(~isnan(RD(:,i)),i),PD(~isnan(RD(:,i)),i));
    corr12=corrcoef(RD(~isnan(RD(:,i)),i)+Cyaw0*R(~isnan(RD(:,i)),i),QD(~isnan(RD(:,i)),i));
    corr13=corrcoef(RD(~isnan(RD(:,i)),i)+Cyaw0*R(~isnan(RD(:,i)),i),PD(~isnan(RD(:,i)),i).*U(~isnan(RD(:,i)),i));
    corr14=corrcoef(RD(~isnan(RD(:,i)),i)+Cyaw0*R(~isnan(RD(:,i)),i),QD(~isnan(RD(:,i)),i).*V(~isnan(RD(:,i)),i));
    
    c_dpdr(i)=corr7(2,1);
    c_dqdr(i)=corr8(2,1);
    c_dpudr(i)=corr9(2,1);
    c_dqvdr(i)=corr10(2,1);
    c_dpdrFCT(i)=corr11(2,1);
    c_dqdrFCT(i)=corr12(2,1);
    c_dpudrFCT(i)=corr13(2,1);
    c_dqvdrFCT(i)=corr14(2,1);
...
%%
figure,
plot(q2p_max,c_dpdr,'r-d','LineWidth',2), hold on
plot(q2p_max,c_dqdr,'r--s','LineWidth',2)
legend('dp','dq')
