...
function transformer_animation_evasion(posx,posy,posz,roll,pitch,yaw,dihedrL,dihedrR,cmd_motor_left,cmd_motor_right,cmd_yaw,forces,moments,speed,time,mass,anim_opts)
...
if exist('anim_opts','var')
    for i=1:2:length(anim_opts)-1
        switch char(anim_opts(i))
            ...
            case 'anim_force_scale', anim_force_scale=cell2mat(anim_opts(i+1));
            case 'anim_moment', anim_moment=char(anim_opts(i+1));
            case 'anim_speed',anim_speed=char(anim_opts(i+1));
            case 'anim_speed_scale',anim_speed_scale=cell2mat(anim_opts(i+1));
...
if ~exist('anim_force_scale','var'), anim_force_scale=0.4; end 
if ~exist('anim_moment','var'), anim_moment='off'; end 
if ~exist('anim_speed','var'), anim_speed='on'; end 
if ~exist('anim_speed_scale','var'), anim_speed_scale=0.05; end 
...
if strcmp(anim_beek,'on')
    ...
    DelFly_x=[ 0,              0, 0            , 0  0     , 0             , 0            , 0     , 0           ];
    DelFly_y=[ 0, -bodywidth/2, bodywidth/2, 0, 0     , -bodywidth/2, bodywidth/2, 0     , 0           ];
    DelFly_z=[ 0, 0             , 0            , 0, -chord, -chord        , -chord       , -chord, -bodylength ];
end
...
%         trecov=0.25;
%         
%         ii1=round(120*(tman))+anim_start+anim_zero;
%         ii2=round(120*(trecov))+anim_start+anim_zero;
        t0anim=-0.125;
        tman=0;
        trecov=0.16;
        
        ii1=round(56*(tman))+anim_start+anim_zero;
        ii2=round(56*(trecov))+anim_start+anim_zero;
        
        % colored traces in xy
        plot3(posx(anim_start:ii1),posy(anim_start:ii1),zmin*ones(size(posz(anim_start:ii1))),'LineWidth',6,'Color',[1 1 0.8]), hold on
        plot3(posx(ii1:ii2),posy(ii1:ii2),zmin*ones(size(posz(ii1:ii2))),'LineWidth',6,'Color',[0.8 0.8 1])
        plot3(posx(ii2:anim_start+anim_length),posy(ii2:anim_start+anim_length),zmin*ones(size(posz(ii2:anim_start+anim_length))),'LineWidth',6,'Color',[0.8 1 0.8])
        
        % colored traces in xz
        plot3(posx(anim_start:ii1),ymax*ones(size(posy(anim_start:ii1))),posz(anim_start:ii1),'LineWidth',6,'Color',[1 1 0.8])
        plot3(posx(ii1:ii2),ymax*ones(size(posy(ii1:ii2))),posz(ii1:ii2),'LineWidth',6,'Color',[0.8 0.8 1])
        plot3(posx(ii2:anim_start+anim_length),ymax*ones(size(posy(ii2:anim_start+anim_length))),posz(ii2:anim_start+anim_length),'LineWidth',6,'Color',[0.8 1 0.8])
        
        % colored traces in yz
        plot3(xmax*ones(size(posx(anim_start:ii1))),posy(anim_start:ii1),posz(anim_start:ii1),'LineWidth',6,'Color',[1 1 0.8])
        plot3(xmax*ones(size(posx(ii1:ii2))),posy(ii1:ii2),posz(ii1:ii2),'LineWidth',6,'Color',[0.8 0.8 1])
        plot3(xmax*ones(size(posx(ii2:anim_start+anim_length))),posy(ii2:anim_start+anim_length),posz(ii2:anim_start+anim_length),'LineWidth',6,'Color',[0.8 1 0.8])
        ...
        % plot force and speed scale
        speed_legend=2; %m/s
        force_legend=mass*9.81; %N
        
        if strcmp(anim_speed,'on')
            scaleS=anim_speed_scale*anim_scale;
            plot3((xmin+0.2)*[1, 1], [ymax, ymax-speed_legend*scaleS], zmin*[1 1],'-m','LineWidth',2), hold on
            text((xmin+0.2), ymax-speed_legend*scaleS, zmin,[' ' num2str(speed_legend) ' m/s'])
        end 
        
        if strcmp(anim_force,'on')
            scaleF=anim_force_scale*anim_scale;
            plot3((xmin+0.4)*[1, 1], [ymax, ymax-force_legend*scaleF], zmin*[1 1],'-b','LineWidth',2), hold on
            text((xmin+0.4), ymax-force_legend*scaleF, zmin,[' ' num2str(force_legend/mass/9.81) ' g'])
        end
        
    elseif strcmp(anim_view,'top')    
        plot3(posx(anim_start:anim_start+anim_length),posy(anim_start:anim_start+anim_length),zmin*ones(size(posz(anim_start:anim_start+anim_length))),'k','LineWidth',1,'MarkerSize',0.5), hold on
%         plot3(posx(anim_start:anim_start+anim_length),ymax*ones(size(posy(anim_start:anim_start+anim_length))),posz(anim_start:anim_start+anim_length),'k:','LineWidth',1,'MarkerSize',0.5)
%         plot3(xmax*ones(size(posx(anim_start:anim_start+anim_length))),posy(anim_start:anim_start+anim_length),posz(anim_start:anim_start+anim_length),'k:','LineWidth',1,'MarkerSize',0.5)
    end
end
...
%     flap_ang=40/180*pi*sin(2*pi*17*i/56);
...
%         scaleS=0.05*anim_scale; % for Science figure 2, 1 m/s ~ 0.05 m
        scaleS=anim_speed_scale*anim_scale;
        plot3([posx(i), posx(i)+speed(1,i)*scaleS], [posy(i), posy(i)-speed(2,i)*scaleS], [posz(i), posz(i)-speed(3,i)*scaleS],'-m','LineWidth',2), hold on
...
%         scaleF=1/(mass*9.81)*0.1*anim_scale; % for Science figure 2, 1g ~ 0.1 m
        scaleF=1/(mass*9.81)*anim_force_scale*anim_scale;
        plot3([posx(i), posx(i)+forces(1,i)*scaleF], [posy(i), posy(i)-forces(2,i)*scaleF], [posz(i), posz(i)-forces(3,i)*scaleF],'-b','LineWidth',2), hold on
