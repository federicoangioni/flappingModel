...
fx_model =  -np.sin(theta_vals) * g - 2 * bx / m * (u_vals - lz * theta_vals + fldd_vals)
...
plt.subplot(3, 1, 1)
plt.plot(TIME, fx_model, linewidth=2, label='Model', linestyle='--')
plt.plot(TIME, np.mean(ACCx, axis=0), label='Flight Data')
plt.title(r'$F_x$ model')
plt.ylabel(r'$\dot{u}$ ($rad/s$)')
plt.legend()
